package com.mateuszb.p2pcommunication.services;
...
import com.mateuszb.p2pcommunication.connection.ChatClient;
import com.mateuszb.p2pcommunication.connection.ChatConnection;
import com.mateuszb.p2pcommunication.connection.ChatServer;
import com.mateuszb.p2pcommunication.data.BaseMessage;
import com.mateuszb.p2pcommunication.data.ChatMessage;
import com.mateuszb.p2pcommunication.data.JoinMessage;
import com.mateuszb.p2pcommunication.data.LeaveMessage;
import com.mateuszb.p2pcommunication.helper.InfoHelper;
import com.mateuszb.p2pcommunication.ui.ChatActivity;
import com.mateuszb.p2pcommunication.ui.PeerActivity;
...
public class P2pChatService extends Service implements WifiP2pManager.PeerListListener, WifiP2pManager.ConnectionInfoListener {
    ...
    public void onConnectionInfoAvailable(WifiP2pInfo p2pInfo) {
        ...
        if (!InfoHelper.areInfosEquals(this.PrevP2pInfo, p2pInfo)) {
            Log.d("P2P", "New info");
    ...
    private MessageReceivedCallback MessageReceiver = new MessageReceivedCallback() {
        @Override
        public void ReceiveMessage(BaseMessage message) {
            if (message instanceof JoinMessage) {
                ClientInfo client = ((JoinMessage)message).getmClient();
                if (!P2pChatService.this.Clients.contains(client)) {
                    P2pChatService.this.Clients.add(client);
                }
            } else if (message instanceof LeaveMessage) {
                ClientInfo client = ((LeaveMessage)message).getmClient();
                if (!P2pChatService.this.Clients.contains(client)) {
